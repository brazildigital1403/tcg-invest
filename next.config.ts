 ...
 * - BLOG (Instagram/TikTok/YouTube): testado em Report-Only, confirmado no
 *   post real "bem-vindo-a-bynx" que o Instagram embed.js e bloqueado sem
 *   `script-src`, e o iframe do embed sem `frame-src` -- mesmo padrao do
...
const CSP = [
  "default-src 'self'",
  // GTM (gated no consentimento) + Turnstile (captcha do cadastro) + embeds do blog (Instagram/TikTok)
  ...
  // youtube-nocookie (facade->iframe do bloco de video) + instagram/tiktok (iframe interno do embed)
