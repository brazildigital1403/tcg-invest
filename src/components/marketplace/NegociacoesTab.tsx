...
const STEPS_COMPRADOR = [
  { status: ['reservado'],                      label: 'Interesse demonstrado', icon: '🤝' },
  { status: ['em_negociacao'],                  label: 'Em negociação',         icon: '💬' },
  { status: ['enviado'],                        label: 'Carta enviada',         icon: '📦' },
  { status: ['concluido'],                      label: 'Recebido!',             icon: '✅' },
]

const STEPS_VENDEDOR = [
  { status: ['reservado'],                      label: 'Comprador interessado', icon: '👀' },
  { status: ['em_negociacao'],                  label: 'Em negociação',         icon: '💬' },
  { status: ['enviado'],                        label: 'Você enviou',           icon: '📦' },
  { status: ['concluido'],                      label: 'Venda concluída!',      icon: '✅' },
]
          ...
          {(nomeContato || whatsapp) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                ...
                {cidadeContato && <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>📍 {cidadeContato}</p>}
              </div>
              {whatsapp && (
      ...
      <div style={{ padding: '0 16px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {role === 'vendedor' && status === 'reservado' && (
          <>
            <div style={{ background: 'rgba(96,165,250,0.06)', border: '1px solid rgba(96,165,250,0.15)', borderRadius: 10, padding: '10px 14px', textAlign: 'center' }}>
              <p style={{ fontSize: 12, color: '#60a5fa' }}>💬 Combine a entrega pelo WhatsApp antes de confirmar o envio</p>
            </div>
        ...
        {role === 'vendedor' && status === 'enviado' && (
          <div style={{ background: 'rgba(168,85,247,0.06)', border: '1px solid rgba(168,85,247,0.2)', borderRadius: 10, padding: '12px 14px', textAlign: 'center' }}>
            <p style={{ fontSize: 13, color: '#a855f7', fontWeight: 600, marginBottom: 3 }}>📦 Carta enviada!</p>
        ...
        {status === 'concluido' && (
          <div style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 10, padding: '12px 14px', textAlign: 'center' }}>
            <p style={{ fontSize: 13, color: '#22c55e', fontWeight: 700 }}>✅ Negociação concluída!</p>
          </div>
        )}
