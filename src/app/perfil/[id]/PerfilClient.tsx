...
/**
 * ★ `inicial` vem do SERVIDOR (04/09/2026). Antes tudo era buscado em
 * `useEffect`, que so roda no browser -- o crawler recebia "Carregando
 * perfil..." e mais nada. Agora os estados que carregam o CONTEUDO nascem
 * preenchidos, entao o mesmo JSX renderiza no SSR e o HTML sai com a colecao.
 *
 * O `useEffect` continua rodando: ele completa o que o servidor nao traz
...
export default function PerfilPage({ inicial }: { inicial?: PerfilPublico | null }) {
  ...
  const [user, setUser]           = useState<any>(inicial?.user ?? null)
  const [listings, setListings]   = useState<any[]>([])
  const [showcase, setShowcase]   = useState<any[]>(inicial?.showcase ?? [])
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <div className="perfil-scroll" style={{ display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 6 }}>
              {showcase.map((card, i) => {
                ...
                // ★ LINK PRA PAGINA DA CARTA (04/09/2026). O perfil listava as
                // cartas mais valiosas e nao linkava NENHUMA -- a pagina
                // inteira tinha 4 hrefs, todos pra ela mesma. Sao ate 6 links
                // internos por perfil, em 370 perfis, apontando pro catalogo,
                // que e o ativo de SEO da casa. `slug` so falta quando a carta
                // do usuario nao casou com o catalogo; ai fica sem link, em
                // vez de virar URL quebrada.
                return card.slug ? (
                  <Link
