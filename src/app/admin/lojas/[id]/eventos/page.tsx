...
export default function AdminEventosLojaPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/lojas/${lojaId}/eventos`, { credentials: 'same-origin' })
  ...
  async function excluir(ev: Evento) {
    ...
    try {
      const res = await fetch(`/api/lojas/${lojaId}/eventos/${ev.id}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      })
      if (!res.ok) {
    ...
    <div style={S.page}>
      <div style={S.topo}>
        <div>
          ...
          <h1 style={S.h1}>Eventos {lojaNome ? `· ${lojaNome}` : ''}</h1>
          <p style={S.sub}>Torneios, ligas, pré-lançamentos e encontros da loja.</p>
        </div>
...
const S: Record<string, CSSProperties> = {
  ...
  topo: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap', marginBottom: 24 },
  voltar: { fontSize: 13, color: 'rgba(255,255,255,0.5)', textDecoration: 'none' },
  h1: { fontSize: 24, fontWeight: 800, margin: '6px 0 4px' },
  sub: { fontSize: 13, color: 'rgba(255,255,255,0.5)', margin: 0 },
  muted: { padding: '32px 0', textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: 14 },
  vazio: { padding: '40px 24px', textAlign: 'center', color: 'rgba(255,255,255,0.5)', fontSize: 14, background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.12)', borderRadius: 14 },
  ...
  itemCard: { display: 'flex', gap: 14, background: '#0d0f14', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 16, alignItems: 'flex-start' },
  ...
  tipoBadge: { fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: 'rgba(245,158,11,0.12)', color: '#f59e0b', textTransform: 'uppercase', letterSpacing: '0.04em' },
  ...
  itemQuando: { fontSize: 13, color: '#f59e0b', margin: '2px 0', textTransform: 'capitalize' },
  ...
  btnPrimary: { background: 'linear-gradient(135deg, #f59e0b, #ef4444)', border: 'none', color: '#000', padding: '10px 18px', borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' },
