...
export default function CartaPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Busca preço no Supabase via API route
      if (data.data?.name) {
        ...
        <div style={{ display: 'flex', gap: 32, marginBottom: 32, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '16px 20px', marginBottom: 20 }}>
              ...
              {price ? (
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.25)', fontStyle: 'italic' }}>
                  Sem dados de preço · <Link href="/" style={{ color: '#f59e0b', textDecoration: 'none' }}>Importe pelo Bynx</Link>
                </p>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <Link href="/" style={{ flex: 1, display: 'block', textAlign: 'center', background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: '#000', padding: '13px', borderRadius: 12, fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>
                🤝 Tenho interesse
              </Link>
              <button
                onClick={handleCopy}
              ...
              {card.attacks.map((atk: any, i: number) => (
                <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {atk.text && <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>{atk.text}</p>}
                  </div>
