...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!accountId) {
      ...
      // Prazo de repasse (14/30 dias). Falha aqui NAO quebra o onboarding —
      // o valor default da Stripe vale e a gente re-aplica depois.
      try {
        await stripe.accounts.update(accountId, {
          settings: {
            payouts: { schedule: { interval: 'daily', delay_days: normalizarPrazo(loja.repasse_prazo) } },
          },
        })
      } catch (e) {
