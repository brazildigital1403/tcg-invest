  ...
  return (
    <AppLayout>
      ...
      {selectedCard && (() => {
        ...
        const ENERGY: Record<string,string> = { Fire:'🔥',Water:'💧',Grass:'🌿',Lightning:'⚡',Psychic:'🔮',Fighting:'👊',Darkness:'🌑',Metal:'⚙️',Dragon:'🐉',Colorless:'⭕',Fairy:'🌸' }
        ...
        const navigateTo = (card: any) => {
          setSelectedCard(card)
          setSelectedCardIndex(cards.indexOf(card))
          setSelectedVariante(pickBestVariante(card))
        }

        return (
          <div onClick={() => setSelectedCard(null)} style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(0,0,0,0.85)', backdropFilter:'blur(10px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
            <div onClick={e => e.stopPropagation()} style={{ background:'#0d0f14', border:'1px solid rgba(255,255,255,0.1)', borderRadius:24, width:'100%', maxWidth:900, maxHeight:'92vh', display:'flex', flexDirection:'column', boxShadow:'0 32px 100px rgba(0,0,0,0.7)', overflow:'hidden' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 20px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'rgba(0,0,0,0.2)', flexShrink:0 }}>
                <button onClick={() => prevCard && navigateTo(prevCard)} disabled={!prevCard}
                ...
                <span style={{ fontSize:11, color:'rgba(255,255,255,0.25)' }}>{cardIdx + 1} de {cards.length}</span>
                <button onClick={() => nextCard && navigateTo(nextCard)} disabled={!nextCard}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', flex:1, overflow:'hidden' }}>
                ...
                <div style={{ padding:20, background:'rgba(255,255,255,0.02)', borderRight:'1px solid rgba(255,255,255,0.06)', display:'flex', flexDirection:'column', alignItems:'center', gap:12, overflowY:'auto' }}>
                  {(c.image_large || c.image_small) && (
                    <img src={c.image_large || c.image_small} alt={c.name} style={{ width:'100%', maxWidth:220, borderRadius:12 }} />
                  )}
              ...
              <div style={{ padding:'16px 24px', borderTop:'1px solid rgba(255,255,255,0.07)', background:'rgba(0,0,0,0.3)', flexShrink:0 }}>
                <p style={{ fontSize:11, color:'rgba(255,255,255,0.4)', marginBottom:10, fontWeight:600 }}>Qual versão você possui?</p>
                <div style={{ display:'grid', gridTemplateColumns:`repeat(${VARIANTES.length}, 1fr)`, gap:8, marginBottom:14 }}>
                  {VARIANTES.map(v => {
                    const isSelected = selectedVariante === v.key
                    const price = fmtBRL(v.med)
                    return (
                      <button key={v.key} onClick={() => setSelectedVariante(v.key)}
                        style={{ padding:'10px 8px', borderRadius:12, cursor:'pointer', fontFamily:'inherit', textAlign:'center', transition:'all 0.15s', background: isSelected ? 'rgba(245,158,11,0.15)' : 'rgba(255,255,255,0.04)', border: isSelected ? '2px solid rgba(245,158,11,0.6)' : '1px solid rgba(255,255,255,0.08)', color: isSelected ? '#f59e0b' : 'rgba(255,255,255,0.5)' }}>
                        <p style={{ fontSize:16, marginBottom:2 }}>{v.icon}</p>
                        <p style={{ fontSize:12, fontWeight:700 }}>{v.label}</p>
                        {price && <p style={{ fontSize:11, color: isSelected ? 'rgba(245,158,11,0.8)' : 'rgba(255,255,255,0.3)', marginTop:2 }}>{price}</p>}
                      </button>
                    )
                  })}
                </div>
                <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
                  <button onClick={() => setSelectedCard(null)} style={{ background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.5)', padding:'11px 24px', borderRadius:12, fontSize:13, cursor:'pointer', fontFamily:'inherit' }}>
                    Fechar
                  </button>
                  <button onClick={() => { handleAddCard({ ...c, _variante: selectedVariante }); setSelectedCard(null) }}
                    style={{ background:'linear-gradient(135deg, #f59e0b, #ef4444)', border:'none', color:'#000', padding:'11px 28px', borderRadius:12, fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:8 }}>
                    + Adicionar à Coleção
